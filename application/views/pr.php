...
<div class="w3-panel w3-card-4 w3-light-grey">
	...
	<div class="w3-row-padding">
		...
		<div class="w3-col m4">
			...
			<div class="w3-bar">
			  ...
			  <button class="w3-bar-item w3-button w3-teal" onclick="Reprint_Doc(1, '<?php echo base_url();?>')"><i class="fa fa-print" aria-hidden="true"></i> Re-Print</button>
			</div>		
		</div>
	</div>
	...
	<div class="w3-row-padding">
		...
		<div class="w3-col m4">
			<label>Type</label>
			<select id="id_pur_type" class="w3-select">
...
<div class="w3-panel w3-card-4 w3-light-grey">
	...
	<div class="w3-row-padding">
		...
		<div id="div_item" class="w3-col m8">
			...
			<select id="idgrn_item" class="w3-select" onchange="read_stk_bal_item('<?php echo base_url()?>',this.value)">
				<?php
					$this->load->view('item_option_list');
				?>
			</select>
		</div>		
	</div>
	...
	<!-- <div class="w3-row-padding">
		...
		<div class="w3-col m4">
			...
			<input id="idgrn_totcost" class="w3-input w3-right-align" type="text" disabled>	
		</div> 							
	</div> -->
...
<script type="text/javascript">
    ...
    function update_GRN(){
        ...
        <?php echo "var url = '". base_url()."index.php/transaction/updatepr';"; ?>

        //alert (JSON.stringify(ReqHdrData));
		...
		$.ajax({
			...
			success: function(data){
                ...
                if (parseInt(data[0]) == 1){                	
                	...
                	var print_url = '<?php echo base_url();?>index.php/print/1/'+data[3];
    ...
    function storeHdrDet() {
        ...
        var _idgrn_freetxt = escape($('#idgrn_freetxt').val());
        var _purtype = $('#id_pur_type').val();
        var _sap_refno = $('#idsap_refno').val();

        //_idgrn_freetxt = _idgrn_freetxt.replace(/'/g,"\'");
        //_idgrn_freetxt = _idgrn_freetxt.replace(/,/g,"\,");

        //_idgrn_freetxt = _idgrn_freetxt.replace(/(?:\r\n|\r|\n)/g, '\\n');

        _idgrn_freetxt.replace(/[\"]/g, '\\"');
        _idgrn_freetxt.replace(/[\\]/g, '\\\\');
        _idgrn_freetxt.replace(/[\/]/g, '\\/');
        _idgrn_freetxt.replace(/[\b]/g, '\\b');
        _idgrn_freetxt.replace(/[\f]/g, '\\f');
        _idgrn_freetxt.replace(/[\n]/g, '\\n');
        _idgrn_freetxt.replace(/[\r]/g, '\\r');
        _idgrn_freetxt.replace(/[\t]/g, '\\t');

        //alert(_idgrn_freetxt);
        ...
        _ReqHdrData[0] = { "remarks": _idgrn_freetxt, "insertuser": "<?php echo $this->session->userdata['logged_in']['username'];?>", "reqdate": _reqdate, "purtype": _purtype, "sap_refno": _sap_refno };
