...
if(count($com_para) > 0){
	for ($row = 0; $row < count($com_para); $row++) {
		...
		$com_name = $this->company->read_selected_company($com_para[$row]['companyno']);
		
		if (isset($com_name)){
			$com_name = $com_name->description;
		}else{
			$com_name = '';
		}
...
$rpt_det = $this->store->read_stock_summary_report($com_para, $item_para);
...
<body>
    ...
    <table style="width: 8.5in;">
        ...
        <tr>
            <td style="text-align: center;"><b>Stock Summary</b></td>
        </tr>
        <tr>
        ...
        <?php
        	...
        	{	
        		echo "<tr>";
        			...
        			echo "<td width=\"20%\">".$row_dtl->itemcode."</td>";
        			echo "<td width=\"40%\">".$row_dtl->description."</td>";
